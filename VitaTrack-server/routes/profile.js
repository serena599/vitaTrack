...
router.get("/user/:user_id", (req, res) => {
  ...
  pool.query(query, [userId], (err, results) => {
    ...
    console.log(
      `User query results: ${results ? results.length : 0} records found`
    );
...
router.put("/user/:user_id", (req, res) => {
  ...
  pool.query(
    ...
    (err, usernameResults) => {
      ...
      pool.query(
        "SHOW COLUMNS FROM user LIKE 'joinedYear'",
        (err, joinedYearResults) => {
          if (err) {
          ...
          if (columnsToAdd.length > 0) {
            ...
            const alterQuery = `ALTER TABLE user ADD COLUMN ${columnsToAdd.join(
              ", ADD COLUMN "
            )}`;
...
router.post("/uploadAvatar", (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    pool.query(
      "UPDATE user SET avatar_path = ? WHERE user_id = ?",
      [avatarUrl, userId],
      (err, result) => {
        if (err) {
...
router.put("/user/:user_id/updatePassword", (req, res) => {
  ...
  pool.query(query, [newPassword, userId], (err, results) => {
    ...
    console.log(
      `Password update result: affectedRows = ${results.affectedRows}`
    );
